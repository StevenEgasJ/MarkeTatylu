...
async function sendMail({ to, subject, text, html, from }) {
  try {
    ...
    const msg = { from: from || process.env.EMAIL_FROM || `no-reply@${process.env.APP_HOST || 'localhost'}`, to: recipients.join(', '), subject, text, html };
...
async function verifyTransporter() {
  try {
    const tr = await getTransporter();
    if (typeof tr.verify === 'function') { await tr.verify(); console.log('✅ Email transporter verified successfully'); return { ok: true }; }
    console.log('⚠️ Email transporter does not expose verify(); assuming configured');
    return { ok: true, info: 'no-verify' };
  } catch (err) {
    console.error('Email transporter verification failed:', err && err.message ? err.message : err); return { ok: false, error: err };
  }
}
