...
router.post('/create-admin', async (req, res) => {
  try {
    ...
    const newUser = new User({ nombre: nombre || 'Administrador', apellido: apellido || '', email: normalizedEmail, passwordHash, cedula: cedula || '', telefono: telefono || '', photo: photo || '', isAdmin: true });
...
router.get('/dbstatus', async (req, res) => {
  try {
    ...
    res.json({ state, productsCount, usersCount, ordersCount, sampleProducts });
  } catch (err) {
