...
async function ensureLogFile() {
  try {
    ...
    try {
      ...
      await fs.writeFile(LOG_FILE, JSON.stringify([]), 'utf8');
    }
  } catch (err) {
...
// Append a metric entry to the log file
async function appendMetric(entry) {
  try {
    ...
    arr.push(entry);
    // cap to last 1000 entries to avoid runaway file growth
...
// POST /metrics/rail - accept JSON metrics from client
...
// GET /metrics/rail - return recent metrics (admin/debug)
router.get('/rail', async (req, res) => {
  try {
    await ensureLogFile();
    const raw = await fs.readFile(LOG_FILE, 'utf8');
    const arr = JSON.parse(raw || '[]');
    // return last 200 entries by default
